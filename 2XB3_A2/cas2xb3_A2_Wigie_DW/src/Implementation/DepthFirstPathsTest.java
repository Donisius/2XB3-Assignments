	...
	void test() throws FileNotFoundException {
        ...
        for(String city : cities) {
        	for(String city2 : adj.get(city)) {
        		DepthFirstPaths testPath = new DepthFirstPaths(cityPaths, city, cities);
        ...
        for (String startCity : startingPoints) {
        	for(String destinationCity : destinationPoints) {
	        	DepthFirstPaths testDFS = new DepthFirstPaths(cityPaths, "boston", cities);
	        	if(testDFS.hasPathTo(destinationCity)) {
	        		Stack<String> pathDFS = (Stack<String>) testDFS.pathTo(destinationCity);
	        		ArrayList<String> testPathDFS = new ArrayList<String>();
	        		for(String e : pathDFS) {
	        			testPathDFS.add(e);
	        		}
	        		System.out.print("DFS path found from: ");
	        		System.out.print(startCity);
	        		System.out.print(" to ");
	        		System.out.print(destinationCity);
	        		System.out.println(":");
	        		System.out.println("**************************************************");
	        		for(String i : testPathDFS) {
	        			System.out.println(i);
	        		}
	        		for(int i = 1; i < testPathDFS.size(); i += 2) {
	        			String startPeg = testPathDFS.get(i - 1);
	        			String endPeg = testPathDFS.get(i);
	        			assert(adj.get(startPeg).contains(endPeg));
	        		}
	        		System.out.println("TEST PATH AGAINST ADJACENCY LIST, DFS PATH VALID");
	        		System.out.println("**************************************************");
	        	}	
        	}
        }
